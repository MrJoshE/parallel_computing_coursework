...
$$
Arithmetic\ Intensity = \frac{number\ of\ FLOPS}{number\ of\ byte\ accesses} = \frac{2 * (M * N * K)}{2 * (N * K + N * K + M)} = \frac{M * N * K}{M * K + N * K + M * N}
$$
Matrix A = M * K matrix, where M are rows and K are columns. 
...
Block size taken from lab 3 `#define BLOCK_SIZE 16`

becuase the algorithm takes `BLOCK_SIZExBLOCK_SIZE` sub-matrix’s the number of rows and columns will be 16.

Therefore the fomula required will be:
$$
\frac{16 * 16 * 16}{16 * 16 + 16 * 16 + 16 * 16} = 5.34
$$
...
I am using sequential addressing in my implementation to speedup the performance
